...
class PodcastTTSProcessor:
    ...
    def __init__(self, tts_config_path: str = None):
        """
        Initialize with TTS configuration
        
        Args:
            tts_config_path: Path to TTS config file, uses default if None
        """
        self.logger = logging.getLogger(__name__)
        
        # Initialize TTS generator
        if tts_config_path:
            self.tts_generator = SimpleTTSGenerator(tts_config_path)
        else:
            # Use default config path
            default_config = Path(__file__).parent.parent / "TTS" / "config" / "tts_config.json"
    ...
    def generate_podcast_audio(
        ...
        self.logger.info(f"Processing TTS script: {Path(tts_script_path).name}")
        
        # Setup output directory
        if output_directory is None:
            output_directory = Path("../../Content/Audio/Generated") / tts_script["episode_info"]["episode_number"]
