"""
JSON Parser and Validator for Gemini Response Files

This module handles parsing and validation of Gemini response files that contain
the new podcast_sections[] format. It extracts audio sections for TTS processing
...
@dataclass
class AudioSection:
    """Represents an audio section that needs TTS processing."""
    section_id: str
...
class GeminiResponseParser:
    """
    Parser for Gemini response files with podcast_sections[] format.
    ...
    def __init__(self):
        ...
        # Required fields for different section types
        self.common_required_fields = {'section_type', 'section_id', 'estimated_duration'}
        self.audio_required_fields = self.common_required_fields | {'script_content', 'audio_tone'}
    ...
    def parse_response_file(self, file_path: Union[str, Path]) -> Dict:
        """
        Parse a Gemini response file and extract the JSON structure.
        
        Args:
            file_path: Path to the Gemini response file
