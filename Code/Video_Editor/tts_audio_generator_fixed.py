...
Updated to work with the new Content/{Series}/{Episode}/Output/Audio/ structure
"""
...
# Try to import FileOrganizer for proper path management
try:
...
class TTSAudioGenerator:
    def __init__(self, output_dir: str = None, episode_series: str = None, episode_name: str = None):
        """
        Initialize TTS Audio Generator
        
        Args:
            output_dir: Directory to save generated audio files (overrides episode-based paths)
            episode_series: Series name for episode structure (e.g., "Joe_Rogan_Experience")
            episode_name: Episode name for episode structure (e.g., "Joe_Rogan_Experience_2331_Jesse_Michels")
        """
        ...
        elif episode_series and episode_name and FileOrganizer:
            # Use new episode structure
            try:
                # Create minimal FileOrganizer to use path methods
                file_organizer = FileOrganizer({})
                audio_path = file_organizer.get_audio_output_path(episode_series, episode_name)
                self.output_dir = Path(audio_path)
            except Exception as e:
                self.logger.warning(f"Could not use episode structure: {e}, falling back to default")
                base_path = Path(__file__).parent.parent.parent
                self.output_dir = base_path / "Content" / "Generated_Audio" / "Narrator"
        else:
        ...
        # Initialize TTS generator with episode-specific output path
        if SimpleTTSGenerator:
            try:
                self.tts_generator = SimpleTTSGenerator(output_dir=str(self.output_dir))
    ...
    def _generate_audio_file(self, text: str, filename: str, segment_type: str) -> str:
        ...
        try:
            ...
            audio_path = self.tts_generator.generate_audio(
                text=cleaned_text,
                output_filename=filename
            )
            
            if audio_path and output_path.exists():
